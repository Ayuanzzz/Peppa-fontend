...
function filterArr(array1, array2) {
  if (array2) {
    return array1.filter(
      (item1) => !array2.find((item2) => item1.id === item2.user_id)
    );
  } else {
    return array1;
  }
...
//全选雇员
const selectAll = () => {
  checkList.value = empListFilter.value.map((emp) => emp.id);
};
...
const handleDelete = (index, row) => {
  removeUp(row.user_id, row.project_id)
    .then((res) => {
      if ((res.status = 200)) {
        ElMessage({
          message: "删除已成功",
...
<template>
  <div class="container">
    ...
    <el-dialog title="添加人员" v-model="showForm" class="dialog">
      ...
      <el-checkbox-group
        v-model="checkList"
        v-for="emp in empListFilter"
        :key="emp.id"
      >
        <el-checkbox :label="emp.id" class="checkbox">{{
          emp.name
        }}</el-checkbox>
      </el-checkbox-group>
      <div class="footer">
